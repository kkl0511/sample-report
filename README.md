...
### 배경
사용자 요청: Driveline 공식 리포트의 변인 중 우리 모델에 채택할만한 게 있을지 검증.
입수 자료: Driveline "Mechanical Composite Scores+" 5 카테고리 20변수 (상대 중요도, Per 1mph 공개)

### 검증 절차

**Step 1 — 변수 매칭 + 추가 추출**: 16/20 매칭 후, 누락 4개를 Uplift 원자료에서 추가 추출
- CoG_Decel, CoG_Decel_AE: `whole_body_center_of_mass_xyz` 시계열 (peak forward velocity − release velocity)
- Elbow_Flex_FP: `{arm_side}_elbow_flexion` at FC frame
- Shoulder_Abd_FP_vertical: `-{arm_side}_shoulder_adduction` at FC frame

**Step 2-3 — 헤드투헤드 (BBL n=169)**:

| 모델 | 변수 | 학습 R² | LOO-CV R² | 평가 |
|---|---|---|---|---|
| **A. v33.21 (현재)** | 10 (메카닉 5 + 신체 5) | 0.361 | **0.267** ★ | baseline |
| B. Driveline 19 메카닉만 | 19 | 0.236 | **0.020** ⚠ | overfit 심각 |
| C. Driveline + 신체 + SD | 28 | 0.455 | 0.183 | overfit |

→ Driveline 통째 채택 거부. 우리 v33.21이 **LOO-CV 일반화에서 압도 우월** (0.267 vs 0.020).

**Step 5 — 개별 변수별 ΔLOO-CV·VIF·p-value**:
채택 기준 ΔLOO>0.005 AND p<0.10 AND VIF<5

| 변수 | ΔLOO-CV | p | VIF | 평가 |
|---|---|---|---|---|
| **CoG_Decel** | **+0.021** ★ | **0.008** | 6.9 | **채택 (압도적 1위)** |
| Hip-Shoulder Sep at FP | +0.007 | 0.061 | 3.3 | max_x_factor와 동일 개념 → 거부 |
| Torso Rotation at FP | +0.009 | 0.060 | 8.0 | Flying Open 결함과 동일 → 거부 |
| Layback | -0.006 | 0.347 | 99.3 | VIF 폭주 — 거부 |
| Stride Length | -0.007 | 0.653 | 65.8 | VIF 폭주 — 거부 |
| 나머지 13개 | -0.013~0.000 | - | - | LOO 하락 또는 다중공선성 |

→ **단 1개 변수 (cog_decel)만 채택**

### 사용자 통찰 검증 (정확)

- **Hip-Shoulder Sep at FP = X-Factor**: Driveline elite 31° vs 우리 max_x_factor 30.96° 정확히 일치. "Peak"라는 표현이 핵심 — peak X-Factor 측정. 우리는 이미 v33.21에 보유.
- **Torso Rotation at FP = Flying Open 개념**: 트렁크가 일찍 회전한 정도 측정 = Uplift 결함 Flying Open. 이미 결함 카드 + P 카테고리에 반영.

### VELO_REGRESSION_v33_22 (11변수)

| 지표 | v33.21 | v33.22 | 변화 |
|---|---|---|---|
| 학습 R² | 0.361 | **0.377** | +0.016 |
| adj R² | 0.321 | **0.332** | +0.012 |
| LOO-CV R² | 0.267 | **0.268** | +0.001 (안정) |

**비표준화 계수**: cog_decel +3.282 km/h per m/s

**표준화 β 1~5순위**:
1. proper_sequence +0.276
2. Grip Strength +0.253
3. Height +0.235
4. max_x_factor +0.214
5. **cog_decel +0.183** ← 신규

**김강연 H2 (2025-11-21) 잔차**:
- v33.21: 실측 146.3, 예측 134.1, 잔차 **+12.2 km/h**
- v33.22: 실측 146.3, 예측 135.3, 잔차 **+11.1 km/h** (1.1 km/h 향상)

### 변경 파일
- `metadata.js` — VELO_REGRESSION_v33_22 + predictMaxVelocity (cog_decel 추가)
- `metadata.js` — EXTRA_VAR_SCORING + PLAUSIBLE_RANGES에 cog_decel 등록
- `cohort_v29.js` — var_distributions cog_decel 추가, C2_FrontLegBlock 카테고리에 매핑
- `extract_uplift_scalars.py` — cog_decel 정식 추출 로직 통합 (whole_body_center_of_mass)
- `app.js` — ALGORITHM_VERSION = 'v33.22', 패치노트 추가

### 향후 권고
- Driveline 5 카테고리 점수를 외부 비교 기준으로 별도 카드 표시 (회귀 비반영, 참고용)
- cog_decel 추출 인프라 — bbl_per_session.csv 정식 컬럼 추가 (현재는 임시 csv 별도)
...
### 사용자 요청 (김강연 선수 리포트 검토)
> 메시지 우선순위 흔들림 + 모순 표현 (정상↔손실, 앞다리 우수↔Late Block, SSC 92↔SSC efficiency 낮음)을 제거하고, 선수·코치 전달용 핵심 카드를 별도로 두라

### v33.12 변경

| Group | 영역 | 핵심 |
|---|---|---|
| **G1** | KINETIC_FAULTS · VISUAL_GUIDE · 마네킹 SVG · KPI 라벨 | LeadKneeCollapse '무너짐' → '신전 약함', LateBlock cause '동작 내 반응 타이밍 지연 (체력 SSC와 별개)', C2 카테고리 '앞다리 형태/각도' (형태 명시), SVG '✅ 키네틱 체인 정상 / 누수 미감지' → '✓ 전체 회전 순서 정상 / 시퀀스 누수 미감지 — 미세 조정은 4단계 참조' |
| **G2** | C5_UpperBodyTransfer target/method | 레이백 185°+ → '현재 수준 유지' (이미 elite), 어깨 IR 4500°/s+ → '코호트 q75 단계적'. 변수별 분기 안내 |
| **G3** | wrist_release_speed · cohort 라벨 | wrist_release_speed: '⚠ 공 속도 아님 — markerless proxy' 캡션. cohort stride/release 라벨에 단위 검증 권고. (단위 검증 자체는 별도 task #18로 추적) |
| **G4** | renderReportHtml | **Athlete Card** 신설 (마지막 섹션). 잘되는 점·가장 먼저 볼 점·선수 큐 ("앞발이 닿으면 빨리 버티고, 몸통 다음에 팔이 나오게 하자.")·오늘 할 훈련·6주 KPI 자동 생성. .kbo-page-head + .kbo-plain + .cat-card 5색 활용 |

### 변경 없음
- 점수 산식·코호트 분포·LITERATURE_OVERRIDE 모두 v33.11과 동일
- v33.11 디자인 시스템 (Light KBO Navy 톤, Theia 컴포넌트) 그대로 유지
- KINETIC_FAULTS id 그대로 (코드 의존, 라벨/cause/coaching만 정리)

### 별도 검증 필요 (task #18 — 후속 작업)
- `stride_mean_m` 김강연 2.04m vs 코호트 mean 1.01m (2배 차이)
- `release_height_m` 김강연 1.17m vs 코호트 mean 0.46m (2.5배 차이)
- raw 산출 코드(extract_uplift_scalars.py + app.js stride_length_trial / release_height_trial)와 cohort_v29.js 산출 단위 일치 여부 검토 필요
...
| **F** | **v0.98 Day 톤** | **index.html `:root` + app.js SVG/inline 색** | **다크 → 라이트 KBO Navy 톤 통째 전환. `--bg-primary #f5f7fa` · `--accent #1F3864` navy · 카테고리 5색(`--output/transfer/leak/injury/control`) + Google Fonts(Inter·Space Grotesk·JetBrains Mono) 로드 + Theia 컴포넌트(`.kbo-page-head/plain/pill/conf/cat-card`) + SVG 어두운 배경 #0b1220 → #ffffff 일괄 교체** |
