...
// v33.22 — Driveline 공식 모델 변인 검증 후 cog_decel 단일 채택 (2026-05-07)
//   [배경] Driveline "Mechanical Composite Scores+" 모델 (5 카테고리 20변수, 상대 중요도·Per 1mph 공개) 입수
//          → 우리 BBL n=169 코호트에서 외부 벤치마크로 정량 검증
//   [Step 1·2 결과] Driveline 19변수 메카닉만 채택 시 LOO-CV R²=0.020 (overfit 심각, 우리 v33.21 0.267 대비 압도 열세)
//                  → 통째 채택 거부, 개별 변수별 정량 평가로 전환
//   [Step 5 개별 검증] v33.21 + 1변수 추가 시 ΔLOO-CV·VIF·p-value 측정 (기준: ΔLOO>0.005 AND p<0.10 AND VIF<5)
//          채택: cog_decel (ΔLOO +0.021, p=0.008, VIF=6.9, 새 차원)
//          거부: Hip-Shoulder Sep at FP — max_x_factor와 동일 개념 (이미 보유)
//          거부: Torso Rotation at FP — Flying Open 결함과 동일 개념 (이미 카드 표시)
//          거부: 나머지 17변수 — VIF 폭주(Layback 99, Stride 66, Pelvis Velo 42, Torso Velo 38) 또는 LOO 하락
//   [추가 추출] extract_uplift_scalars.py — whole_body_center_of_mass 시계열에서 cog_decel 산출
//          정의: peak forward velocity − velocity at release (m/s)
//          코호트 평균 1.02 m/s vs Driveline 엘리트 1.61 m/s (고교 코호트는 Block 효율 낮음 — 정상)
//   [VELO_REGRESSION_v33_22] 11변수 (v33.21 10 + cog_decel)
//          R² 0.361 → 0.377 (+0.016), adj R² 0.321 → 0.332 (+0.012), LOO-CV 0.267 → 0.268 (안정)
//          비표준화 계수: cog_decel +3.282 km/h per m/s (단위 1m/s 차이 = 약 3.3km/h)
//          표준화 β 5순위: cog_decel +0.183 (1순위 proper_seq +0.276, 2위 grip +0.253, 3위 height +0.235, 4위 X-Factor +0.214)
//   [김강연 H2 검증] 실측 146.3 km/h
//          v33.21: 예측 134.1, 잔차 +12.2  →  v33.22: 예측 135.3, 잔차 +11.1 (1.1 km/h 향상)
//   [Driveline 외부 검증의 함의] 그들의 5 카테고리 점수는 외부 비교 기준으로 별도 표시 가능 (회귀 비반영)
const ALGORITHM_VERSION = 'v33.22';
const ALGORITHM_DATE    = '2026-05-07';
