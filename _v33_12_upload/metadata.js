...
const KINETIC_FAULTS = [
  ...
  {
    // ★ v33.12 — 라벨/표현 정리 (사용자 피드백 2026-05-06)
    //   기존 'Lead Knee Collapse' = 무릎이 무너진다는 어감 → 형태/각도 평가 카테고리(C2 78점 우수)와 충돌
    //   실제 발동 조건은 FC→BR 무릎 신전 변화가 -22.2° 미만일 때 (각도 변화량). 일반적 케이스는 '늦은 신전' 또는 '블록 반응 약함'.
    //   ID는 코드 의존이 있어 'LeadKneeCollapse' 유지하되 라벨·cause·coaching은 코칭 친화적 표현으로
    id: 'LeadKneeCollapse', stage: 2, severity: 'high',
    label: '앞다리 신전 약함 (Block Extension Weak)',
    detect: m => m.lead_knee_ext_change_fc_to_br != null && m.lead_knee_ext_change_fc_to_br < -22.2,
    cause: '앞다리 ecc 부족 — FC→BR 동안 무릎이 펴지지 못하고 더 굽어짐. (체력 SSC 점수와 별개로 투구 동작 내 적용 능력)',
    coaching: '앞발 닿은 직후 무릎이 펴지는 방향(stiff하게 버티며 신전)으로 반응. "더 펴려" 하지 말고 "무너지지 않게 빠르게 단단해지는 느낌"',
  ...
  },
  {
    ...
    label: '블로킹 반응 타이밍 지연 (Late Block)',
    ...
    // ★ v33.12 — 표현 정리: 체력 SSC 점수와 충돌하지 않도록 "동작 내 적용 타이밍"으로 재해석
    cause: '투구 동작 내 앞다리 반응 타이밍 지연 — 측정된 SSC 능력은 강점이라도 착지 직후 블로킹으로 빠르게 전환되는 타이밍이 늦음',
    coaching: '앞발이 닿은 즉시 단단해지기 (stiffen on contact). "더 펴려" 하지 말고 "빠르게 버티는 느낌"',
    drills: ['Plyo 단일 다리 점프', 'Contrast training', 'Reactive block drill', 'Drop landing → 즉시 throw'],
  },
  // ── 단계 3 (분리 형성) ──
  {
...
const FAULT_VISUAL_GUIDE = {
  ...
  LeadKneeCollapse: {
    // ★ v33.12 — 'Knee Collapse' 용어 부적절 (사용자 피드백): 카테고리 점수와 충돌, 코칭 친화적이지 않음
    title: '앞다리 신전 약함 (Block Extension Weak)',
    image: 'assets/fault_images/KneeCollapse.png',
    desc: '앞발착지(FC)~볼릴리즈(BR) 구간에서 무릎이 의도와 달리 구부러지는 경우. 형태(C2 카테고리 점수)는 양호하더라도 동작 내 신전 능력이 부족할 수 있음. 지면 에너지가 상체로 전달되는 효율에 영향.',
    physical: '앞다리 근력 (둔부·허벅지·햄스트링), 코어 안정성',
  },
  SoftBlock: {
    title: '블록 부드러움 (Soft Block)',
    image: 'assets/fault_images/KneeCollapse.png',
    desc: '앞다리 block이 부드러워(ecc 약함) COG 감속이 부족한 상태. 회전 전환이 비효율적이 됨. 형태(각도 유지)와는 별개의 dynamic 평가.',
    physical: '앞다리 eccentric 근력, 코어 안정성',
  },
  LateBlock: {
    title: '블로킹 반응 타이밍 지연 (Late Block)',
    image: 'assets/fault_images/KneeCollapse.png',
    desc: '체력 검사상 SSC 반응성이 좋더라도 투구 동작 내에서 앞다리 블록 반응이 늦어지는 경우. 빠른 reactive block 부재.',
...
const CATEGORY_TRAINING_RECS = {
  ...
  'C5_UpperBodyTransfer': {
    ...
    // ★ v33.12 — 사용자 피드백: 레이백 185°+ 목표는 이미 elite 범위(180°+) 선수에게 부적절.
    //   현재 수준 유지 + 약점 변수 우선 보강으로 표현. 어깨 IR도 MLB 4500을 고교 6주 목표로 제시 부적절 → 코호트 단계적
    target: '레이백 180°+ 현재 수준 유지 · 몸통→팔 lag 40~60ms 범위 안정화 · 어깨 IR 속도 코호트 q75 도달 (단계적)',
  },
...
const CATEGORY_DETAILS = {
  ...
  "C2_FrontLegBlock": {"name":"앞다리 형태/각도 유지 (Block Form)","short_desc":"FC→BR 무릎 각도 유지력 평가 (반응 타이밍은 별도)","meaning":"앞다리(블록)의 형태(form) 평가 — 'FC 시점 무릎 각도가 BR까지 유지되는가'. 더 펴거나 굽힘이 아닌 '각도 유지'가 평가 기준. ★ 이는 '형태'만 본 평가이며, '착지 후 빠르게 버티는 반응 타이밍'은 4단계 fault 진단에서 별도로 평가됨 (Late Block).","method":"FC→BR Δ각도(음수만 감점, 0 이상은 만점), 몸 감속도(블록 강도), SSC 시간, 앞 hip 굴곡·신전속도의 백분위 평균","interpretation":[["75-100","형태 우수 — 무릎 각도 유지·블록 단단"],["50-75","평균 — 약한 굽힘 경향"],["25-50","형태 부족 — 신전 약화 가능성 ⚠"],["0-25","형태 심각 — 명백한 신전 약화, ecc 강화 권장"]],"coaching":"FC 순간 앞다리 각도를 BR까지 유지하라 — 더 펴려 하기보다 단단히 버텨라. 별도로 '착지 직후 빠른 반응 타이밍'은 reactive block drill로 보강","reference":"Driveline lead leg, Wood-Smith (2019)"},
  ...
  "C5_UpperBodyTransfer": {"name":"팔 에너지","short_desc":"레이백·전달율·lag·어깨 IR 속도 통합 평가 (변수별 분리 해석)","meaning":"몸통에서 팔로 에너지가 어떻게 전달·증폭되어 공으로 가속되는가. 네 가지 통합 평가: ① 레이백(어깨 외회전 max) — 몸통 에너지 흡수 ROM, ② 몸통→팔 전달 효율(arm_trunk_speedup) — 팔이 몸통보다 얼마나 빨리 회전하는지의 비율, ③ 몸통→팔 시간차(lag) — 전달 타이밍, ④ 어깨 내회전 최대 속도 — 최종 공 가속 단계의 폭발력. ★ 변수별로 강·약점이 갈릴 수 있으므로 종합 점수 외에 변수별 진단 동시 참조 권장.","method":"max_shoulder_ER_deg (Gaussian optimal 185°, sigma 12 — Driveline elite. 이미 elite 범위라면 추가 증대 목표가 아니라 현재 수준 유지) + arm_trunk_speedup (Gaussian optimal 1.88, sigma 0.2 — 20명 elite) + trunk_to_arm_lag_ms (LAG optimal 45ms, sigma 25 — 40~60ms 범위 안정화 우선) + shoulder_ir_vel_max (코호트 percentile 기반 단계적 목표. Driveline elite 4500+는 프로 기준이며 고교에서는 q75 도달이 1차 목표)","interpretation":[["75-100","팔 에너지 우수 — 4요소 모두 양호"],["50-75","평균 — 변수별 강·약점 분리 검토 (예: 레이백은 좋고 lag·IR속도는 보강 필요 같은 패턴 흔함)"],["25-50","전달 부족 — 팔꿈치 의존 위험 ⚠. 단 변수별로 차이 있으니 약점 변수 우선"],["0-25","전달 심각 — 부상 위험 + 트렁크 활성화 우선"]],"coaching":"몸통 에너지를 어깨로 받고 팔로 넘겨라. ★ 변수별 분기: 레이백이 이미 elite(180°+)면 더 늘리려 하지 말고 lag 안정화·IR 속도 점진 강화 우선. Connected throw drill, 메디신볼 회전 던지기, 어깨 ER ROM(가동성 유지)·IR 속도(점진 부하)","reference":"Werner (2008), Wood-Smith (2019), Driveline OnBaseU"},
...
const OUTPUT_VS_TRANSFER_VAR_META = {
  ...
  'wrist_release_speed':   { name: '★ 손목 릴리스 속도',         unit: 'm/s',  hint: '★ 출력 통합 — 모든 회전·전달의 최종 결과. ⚠ 공 속도 아님 — 손목 관절 중심(wrist_jc) markerless proxy. 측정 구속(km/h)과 별개의 메카닉 출력 지표' },
  // ── 전달(TRANSFER) — 13변수 ──
